...
interface RecadvTableRowProps {
    recadv: PricatsResponse
    sendRecadv: (pricat: PricatsResponse) => void
}
function RecadvTableRow(props: RecadvTableRowProps) {

    const navigate = useNavigate();
    const {store} = useContext(Context);

    function displaySubmit(){
        if(props.recadv.documentStatus == 'IMPORTED' && props.recadv.senderId === store.user.gln){
            return true;
        }
        return false;
    }
    ...
    function navigateToPricat(){
        navigate(`/recadvs/${props.recadv.documentId}`);
    }
...
export default observer(RecadvTableRow)